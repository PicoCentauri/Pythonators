...
def RenShareTargetOpt(data,
    ...
    model.nuclearCapacity = pyo.Var(domain=pyo.NonNegativeReals)
    ...
    model.nuclearGen = pyo.Var(model.i, domain=pyo.NonNegativeReals)
    ...
    def energyBalance_rule(model, i):
        return data['demand'].iat[i] + model.curtailment[i] + model.charge[i] == model.renGen[i] + model.discharge[i] + model.nuclearGen[i]
    ...
    def nuclearGen_rule(model, i):
        return model.nuclearGen[i] == (model.nuclearCapacity + installedNuclearCapacity)
    ...
    model.nuclearGen_rule = pyo.Constraint(model.i, rule=nuclearGen_rule)
...
def get_values(model, data):
    ...
    return renShare, curtailed, renGen, nuclearGen
...
def run(data,
    ...
    model = RenShareTargetOpt(data=data,
                              ...
                              installedGasCapacity=installedNuclearCapacity,
    ...
    renShare, curtailed, renGen, nuclearGen = get_values(model,data)
