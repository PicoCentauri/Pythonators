...
def RenShareTargetOpt(data,
    ...
    def energyBalance_rule(model, i):
        return data['demand'].iat[i] + model.curtailment[i] + model.charge[i] == model.renGen[i] + model.discharge[i] + model.gasGen[i]
    ...
    def renShare_rule(model, i):
        return model.renShare[i] == 1 - model.gasGen[i] / data['demand'].iat[i]
...
def get_values(model, data):
    ...
    return renShare, curtailed, renGen, gasGen
...
def run(**params):
    ...
    renShare, curtailed, renGen, gasGen = get_values(model, params["data"])
